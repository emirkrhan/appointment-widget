  ...
  function mountWidget () {
    ...
    async function sendMessage (e) {
      ...
      try {
        ...
        const responseBody = await response.text() // Detaylı hata için response'u oku

        if (!response.ok) {
          console.error(
            'Webhook isteği başarısız oldu:',
            response.status,
            response.statusText,
            responseBody
          )
        ...
        isLoading = false
        console.log('Mesaj başarıyla gönderildi', responseBody)
      } catch (error) {
